...
# 加载字典
def load_char_vocab():
    path = os.path.join(os.path.dirname(__file__), '../input/vocab.txt')   # 单个字
...
# 加载词典
def load_word_vocab():
    path = os.path.join(os.path.dirname(__file__), '../output/word2vec/word_vocab.tsv')   # 有词语
    vocab = [line.strip() for line in open(path, encoding='utf-8').readlines()]
    word2idx = {word: index for index, word in enumerate(vocab)}
    idx2word = {index: word for index, word in enumerate(vocab)}
    return word2idx, idx2word
...
def load_char_data(file, data_size=None):
    ...
    df = pd.read_csv(path,sep='\t',names=['sentence1','sentence2','label'])
...
def load_char_word_static_data(file, data_size=None):   # 读入训练数据文件 三列：s1 s2 label
    ...
    p_c_index, h_c_index = char_index(p, h)   # p转化成每个字在词表里的index位置向量

    p_seg = map(lambda x: list(jieba.cut(x)), p)
    h_seg = map(lambda x: list(jieba.cut(x)), h)

    p_w_vec = list(map(lambda x: w2v(x, model), p_seg))   # 用模型预测对p分词后 每个词的向量
    h_w_vec = list(map(lambda x: w2v(x, model), h_seg))

    p_w_vec = list(map(lambda x: w2v_process(x), p_w_vec))   # 每个词向量处理成指定长度
...
def load_char_word_dynamic_data(path, data_size=None):
    ...
    p_word_index, h_word_index = word_index(p_seg, h_seg)   # p分词后 查词典获取每个词对应的index位置向量
...
if __name__ == '__main__':
    ...
    load_all_data('../input/TRAIN_ALL.csv', data_size=100)